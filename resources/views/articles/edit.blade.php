...
<form method="POST" action="{{ route('article.update', $article) }}">
    ...
    <div class="mb-4">
        ...
        <div class="mt-1 relative rounded-md shadow-sm">
            <input id="title" name="title" class="form-input block w-full sm:text-sm sm:leading-5"
                placeholder="Title of the article" value="{{ $article->title }}" required>
        </div>
    </div>
    <div class="mb-4">
    ...
    <div class="mb-4">
        ...
        <select id="status" name="active"
            class="mt-1 form-select block w-full pl-3 pr-10 py-2 text-base leading-6 border-gray-300 focus:outline-none focus:shadow-outline-blue focus:border-blue-300 sm:text-sm sm:leading-5">
            <option value="1" {{ $article->active == true ? 'selected' : '' }}>Active</option>
            <option value="0" {{ $article->active == false ? 'selected' : '' }}>Inactive</option>
        </select>
    </div>
    <div class="mb-4">
